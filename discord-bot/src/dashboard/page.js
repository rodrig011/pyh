  ...
  <div class="card">
    ...
    <div class="signal-layout">
      ...
      <div class="side-col">
        ...
        <div class="section">
          <div class="section-head"><span class="section-title">Quantitative settlement model</span><span class="badge" id="quantStatus">—</span></div>
          <p class="section-help">Computed by code from distance, time, volatility, liquidity and available order flow — never generated by an LLM.</p>
          <div class="grid">
            <div class="stat"><div class="label">Fair P(YES)</div><div class="value" id="fairYes">—</div></div>
            <div class="stat"><div class="label">Fair P(NO)</div><div class="value" id="fairNo">—</div></div>
            <div class="stat"><div class="label">YES net edge</div><div class="value" id="yesEdge">—</div></div>
            <div class="stat"><div class="label">NO net edge</div><div class="value" id="noEdge">—</div></div>
          </div>
          <div class="live-note" id="quantDetails">Waiting for a complete quantitative read.</div>
        </div>
...
(function () {
  ...
  function paintLiveTrading(lt) {
    ...
    if (!orders.length) {
      ...
      list.innerHTML = orders.map(function (o) {
        ...
        } else if (o.status === 'unfilled' || o.status === 'rejected') {
          right = '<span class="down">' + (o.status === 'unfilled' ? 'NO FILL' : 'REJECTED') + '</span>';
        } else if (o.status === 'partial') {
          right = '<span style="color:var(--amber)">PARTIAL FILL</span>';
        } else if (o.status === 'filled') {
          right = '<span class="up">FILLED</span>';
        } else {
        ...
        return '<div class="order-row">' +
          '<div class="order-left">' +
            ...
            '<span>' + (Number.isFinite(o.requestedContracts) ?
              (Number.isFinite(o.filledContracts) ? o.filledContracts + '/' : '') + o.requestedContracts + 'x @ ' +
                (Number.isFinite(o.fillCents) ? o.fillCents.toFixed(1) : o.limitCents) + '¢' : '') + '</span>' +
            forced +
          '</div>' +
  ...
  function paint(data) {
    ...
    var signedCents = function (n) {
      return Number.isFinite(n) ? (n >= 0 ? '+' : '') + n.toFixed(1) + '¢' : '—';
    };
    document.getElementById('fairYes').textContent = fmtPct(data.fairPYes);
    document.getElementById('fairNo').textContent = fmtPct(data.fairPNo);
    document.getElementById('yesEdge').textContent = signedCents(data.quantYesEdgeCents);
    document.getElementById('noEdge').textContent = signedCents(data.quantNoEdgeCents);
    var quantBadge = document.getElementById('quantStatus');
    if (data.quantStatus === 'ok') {
      quantBadge.textContent = 'GRADE ' + (data.confidenceGrade || '—') + ' · ' + Math.round(data.confidenceScore || 0) + '/100';
      quantBadge.className = 'badge ' + ((data.confidenceScore || 0) >= 70 ? 'up' : 'amber');
    } else {
      quantBadge.textContent = 'INSUFFICIENT DATA';
      quantBadge.className = 'badge down';
    }
    var quantDetails = [];
    if ((data.quantMissing || []).length) quantDetails.push('Missing: ' + data.quantMissing.join(', '));
    if ((data.quantPenalties || []).length) quantDetails.push('Penalties: ' + data.quantPenalties.join(' · '));
    if ((data.quantCaps || []).length) quantDetails.push('Caps: ' + data.quantCaps.join(' · '));
    document.getElementById('quantDetails').textContent = quantDetails.length ? quantDetails.join(' | ') : 'All required quantitative inputs are available.';
