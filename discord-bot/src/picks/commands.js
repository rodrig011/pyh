    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((option) =>
          option
            .setName('profile')
            .setDescription('Signal profile used for live opportunities')
            .addChoices(
              { name: 'careful — strongest signals only', value: 'careful' },
              { name: 'scalp — thinner edges and later entries', value: 'scalp' },
              { name: 'always — every model-approved opportunity', value: 'always' },
            ),
        ),
    )
    .addSubcommand((sub) =>
...
export async function handlePicks(interaction, { store, config, deps = {} }) {
  ...
  if (sub === 'live') {
    ...
    const requestedProfile = interaction.options.getString('profile');
    ...
    if (requestedProfile && !LIVE_PROFILES[requestedProfile]) {
      return interaction.editReply('Unknown live signal profile. Choose careful, scalp, or always.');
    }
    if (requestedProfile) state = { ...state, profile: requestedProfile };
    if (Number.isFinite(limit) || requestedProfile) store.putRiskState(state);
    ...
    if (action === 'arm') {
      ...
      if (Number(trading.forceTradesPerWindow) > 0) {
        return interaction.editReply(
          '❌ **Live trading refused.** Forced profiles and activity quotas are paper-only. ' +
            'Set `KALSHI_FORCE_TRADES_PER_WINDOW=0`; ' +
      ...
      return interaction.editReply(
        [
          ...
          `_Profile: **${activeProfile.label}**${activeProfile.liveSignalsOnly ? ' — no forced entries' : ''}._`,
          '',
    ...
    // Name the persisted selection explicitly: arming and choosing how the
    // signal loop reads the board are separate acts.
    const profileLine = profileName && !LIVE_PROFILES[profileName]
      ? `⚠️ _Live profile "${profileName}" does not exist — running as careful instead._`
      : `_Profile: **${activeProfile.label}**${activeProfile.liveSignalsOnly ? ' — no forced entries' : ''}._`;
    return interaction.editReply(
      [
        summary.killed
